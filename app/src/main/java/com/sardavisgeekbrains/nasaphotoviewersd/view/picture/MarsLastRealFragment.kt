...
class MarsLastRealFragment : Fragment() {
    ...
    private fun openFragment(fragment: Fragment) {
        ...
        private fun render(appState:PictureOfTheDayAppState){
            ...
            when(appState){
                ...
                is PictureOfTheDayAppState.SuccessMars->{
                    while(appState.serverResponseData.photos.isEmpty()){
                        viewModel.sendMarsRequest(dateActual.toString())
                        @RequiresApi(Build.VERSION_CODES.O)
                        dateActual = today.minusDays(i).format(formatter)
                        i++
                    }
        ...
        companion object{
            fun newInstance():MarsLastRealFragment{
                return MarsLastRealFragment()
            }
        }
